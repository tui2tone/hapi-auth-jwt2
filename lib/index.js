 ...
 * "implementation details" (methods) such as authenicate, h & raiseError
...
internals.implementation = function (server, options) {
  ...
  return {
    ...
    authenticate: async (request, h) => {
      ...
      if (options.key && typeof options.validateFunc === 'function') {
        ...
        return new Promise((resolve, reject) => {
          keyFunc = (internals.isFunction(options.key)) ? options.key : function (decoded_token, callback) {
          ...
          keyFunc(decoded, function (err, key, extraInfo) {
            ...
            if (err) {
              return reject(raiseError('wrap', err));
            }
            if (extraInfo) {
                ...
                function (verify_err, verify_decoded) {
                  if (verify_err) {
                    ...
                    if (keysTried >= keys.length) {
                      ...
                      return reject(raiseError('unauthorized', err_message, tokenType));
                    }
                    ...
                    return options.validateFunc(verify_decoded, request,
                      function (validate_err, valid, credentials) { // bring your own checks
                        if (validate_err) {
                          return reject(raiseError('wrap', validate_err));
                        }

                        if (!valid) {
                          return reject(raiseError('unauthorized','Invalid credentials', tokenType))
                        } else {
        ...
        return options.verifyFunc(decoded, request,
          function (verify_error, valid, credentials) {
            ...
            if (!valid) {
              return raiseError('unauthorized', 'Invalid credentials', tokenType);
            } else {
    ...
    /**
     * h is an Optional method called if an options.hFunc is set.
     * @param {Object} request - the standard route handler request object
     * @param {Object} reply - the standard hapi reply interface ...
     * after we run the custom options.hFunc we reply.continue to execute
     * the next plugin in the list.
     * @returns {Boolean} true. always return true (unless there's an error...)
     */
    h: function (request, h) {
      if (options.hFunc && typeof options.hFunc === 'function') {
        options.hFunc(request, reply, function (err) {
          if (err) {
